...
export const registrarCompra = async (req, res) => {
  try { 
    const {  id_empleado, fecha_compra, total_compra, precio_unitario  } = req.body;
 ...
 //Controlador de eliminación de datos de compras
export const eliminarCompra = async (req, res) => {
  try {
    ...
    if (result.affectedRows === 0) {
      return res.status(404).json({
        mensaje: `Error al eliminar los datos. ID ${id_compra} no encontrado.`,
      });
    }
...
// Actualizar una compra parcialmente (PATCH)
export const actualizarCompraPatch = async (req, res) => {
  try {
    const {id_compra} = req.params;
    const datos = req.body;
    const [result] = await pool.query("UPDATE Compras SET ? WHERE  id_compra = ?", [datos, id_compra]);
